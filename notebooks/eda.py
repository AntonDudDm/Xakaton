...
# =============================================================================
# 19. FINAL INSIGHTS + FEATURE SHORTLIST + BUSINESS SUMMARY
# =============================================================================

print("\n" + "=" * 80)
print("19. FINAL INSIGHTS FOR METHODISTS AND TUTORS")
print("=" * 80)

FINAL_SHORTLIST_EARLY = [
    "timezone",
    "time_to_first_action_days",
    "time_to_first_answer_days",
    "answer_first_7d_count",
...
FINAL_SHORTLIST_EARLY = [c for c in FINAL_SHORTLIST_EARLY if c in df.columns]

print("\nFinal early feature shortlist:")
for col in FINAL_SHORTLIST_EARLY:
    print(f"  {col}")

print("\n" + "-" * 80)
print("FINAL ANALYTICAL INSIGHTS")
print("-" * 80)

print("""
1. Главный переносимый сигнал оттока связан не с профилем студента,
   а с тем, вошёл ли он в учебный ритм на раннем этапе.

2. Наиболее важные ранние признаки:
   - time_to_first_action_days
   - time_to_first_answer_days
   - answer_first_7d_count
   - answer_first_14d_count
...
3. Это означает:
   - если студент поздно начал курс;
   - если у него долго нет первого ответа;
   - если в первые 7–14 дней почти нет решений,
   то риск оттока заметно повышается.

4. Course-level признаки дают дополнительный, но вторичный сигнал:
   - course_tasks_per_lesson
   - course_required_tasks_per_lesson
   - course_unique_tasks_per_lesson
   Их можно интерпретировать как индикаторы нагрузки курса,
   но вывод по ним нужно считать осторожным из-за малого числа курсов.

5. timezone можно оставить как сегментационный контекст,
   но не как основной драйвер риска.

6. Методически важный вывод:
   - старая почти идеальная модель была завышена из-за leakage;
   - честная leakage-free постановка даёт более скромное,
     но реально переносимое качество;
   - значит ранний риск-скоринг в LMS действительно возможен.
""")

print("\n" + "-" * 80)
print("BUSINESS SUMMARY FOR TUTORS")
print("-" * 80)

print("""
Для тьюторов:

1. Главная группа раннего риска — студенты, которые фактически не стартовали.
   Признаки:
   - поздний первый action;
   - нет первого ответа;
   - ноль или почти ноль ответов в первые 7–14 дней.

2. Это означает, что тьюторское вмешательство должно происходить очень рано,
   а не после накопления большого числа пропусков.

3. Практические триггеры:
   - нет ответа в первые 7 дней;
   - нет ответа в первые 14 дней;
   - слишком поздний первый action;
   - сочетание "нулевая активность + высокий риск score".

4. Рекомендуемые действия:
   - персональное напоминание;
   - сообщение с простым первым шагом;
   - быстрый контакт тьютора;
   - перевод в список приоритетного сопровождения.
""")

print("\n" + "-" * 80)
print("BUSINESS SUMMARY FOR METHODISTS")
print("-" * 80)

print("""
Для методистов:

1. Основной механизм оттока — не вход в учебный ритм на старте курса.

2. Дополнительная рабочая гипотеза:
   более высокая плотность задач на урок
   может быть связана со снижением completion.

3. Поэтому стоит отдельно проверять:
   - число задач на урок;
   - число обязательных задач;
   - сложность первых двух недель курса;
   - не слишком ли тяжёл вход в курс.

4. Практический смысл:
   курс может терять часть студентов не только из-за слабой мотивации,
   но и из-за перегруза или слишком сложного старта.

5. Рекомендуемые действия:
   - упростить первый вход в курс;
   - снизить перегруз в начале;
   - вынести часть задач из обязательного блока;
   - проверить курсы с высокой плотностью заданий отдельно.
""")
...
print(f"""
Выбранная честная ранняя модель:
- train: M1
- test : M2

Ключевой вывод:
- даже после удаления leakage модель сохраняет полезный переносимый сигнал;
- значит систему раннего выявления риска можно использовать на практике;
- основной объект интервенции — студент, который не вошёл в ритм в первые 1–2 недели.
""")

# =============================================================================
# 20. EXPORT PREDICTIONS TO prediction.parquet
# =============================================================================
...
print("\n" + "=" * 80)
print("20. EXPORT PREDICTIONS")
print("=" * 80)

# ---- базовые признаки выбранной честной ранней модели ----
PRED_BASE_FEATURES = [
    "timezone",
    "time_to_first_action_days",
    "time_to_first_answer_days",
...
PRED_BASE_FEATURES = [c for c in PRED_BASE_FEATURES if c in df_infer.columns]
print("\nBase features for inference:")
for col in PRED_BASE_FEATURES:
    print(f"  {col}")

# ---- готовим pandas-таблицу для модели ----
infer_pd = df_infer.to_pandas().copy()

# categorical
if "timezone" in infer_pd.columns:
    infer_pd["timezone"] = infer_pd["timezone"].astype(str).fillna("missing")

# numeric
for col in PRED_BASE_FEATURES:
    if col != "timezone":
        infer_pd[col] = pd.to_numeric(infer_pd[col], errors="coerce")

# ---- missing flags ----
pred_features_final = PRED_BASE_FEATURES.copy()

for col in PRED_BASE_FEATURES:
    if col == "timezone":
        continue
    miss_col = f"{col}__is_missing"
    infer_pd[miss_col] = infer_pd[col].isna().astype(int)
    pred_features_final.append(miss_col)

# ---- controlled fill for numeric early features ----
FILL_999_COLS = [
    "time_to_first_action_days",
    "time_to_first_answer_days",
]
...
for col in FILL_999_COLS:
    if col in infer_pd.columns:
        infer_pd[col] = infer_pd[col].fillna(999.0)

for col in FILL_ZERO_COLS:
    if col in infer_pd.columns:
        infer_pd[col] = infer_pd[col].fillna(0.0)

# timezone missing fallback
if "timezone" in infer_pd.columns:
    infer_pd["timezone"] = infer_pd["timezone"].fillna("missing").astype(str)

print("\nFinal inference features:")
for col in pred_features_final:
    print(f"  {col}")

# ---- score ----
chosen_model = chosen_res["model"]

pred_proba_completion = chosen_model.predict_proba(infer_pd[pred_features_final])[:, 1]
infer_pd["pred_proba_completion"] = pred_proba_completion
infer_pd["risk_score"] = 1.0 - pred_proba_completion

# ---- risk groups ----
infer_pd["risk_group"] = pd.cut(
    infer_pd["risk_score"],
    bins=[-np.inf, 0.30, 0.50, 0.70, np.inf],
    labels=["low", "moderate", "high", "critical"],
)

# ---- финальная таблица для выгрузки ----
keep_cols = [
    c for c in [
        "user_id",
        "users_course_id",
...
prediction_pd = infer_pd[keep_cols].copy()
prediction_pd = prediction_pd.sort_values("risk_score", ascending=False).reset_index(drop=True)

# ---- parquet export ----
PRED_PATH = OUTPUT_DIR / "prediction.parquet"
pl.from_pandas(prediction_pd).write_parquet(PRED_PATH)

print(f"\nSaved: {PRED_PATH}")
print("\nPrediction parquet preview:")
print(prediction_pd.head(10).to_string(index=False))